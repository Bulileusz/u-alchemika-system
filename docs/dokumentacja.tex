...
\begin{titlepage}
  ...
  \vspace{0.5cm}
  {\large Podstawy i systemy baz danych \par}
  \vspace{2.5cm}
  ...
  \vfill
  {\small Projekt realizowany w ramach kursu \emph{Podstawy i systemy baz danych} \par}
\end{titlepage}
...
\texttt{capacity} & INTEGER & NOT NULL, $>0$ & Liczba gości \\
...
\texttt{guests} & INTEGER & NULL, $>0$ & Liczba gości \\
...
\begin{itemize}[nosep]
  \item \texttt{PositiveIntegerField} -- gwarantuje \texttt{capacity $>$ 0} i~\texttt{guests $>$ 0},
...
implementację więzów integralności (CASCADE, SET NULL, UNIQUE, CHECK),
